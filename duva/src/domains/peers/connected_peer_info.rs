    ...
    pub(crate) fn decide_peer_kind(&self, my_repl_id: &ReplicationId) -> PeerState {
        match (my_repl_id, &self.replid) {
            ...
            (_, ReplicationId::Undecided) => {
                PeerState::new(self.id.clone(), self.hwm, my_repl_id.clone(), NodeKind::Replica)
            },
            // I am undecided - adopt peer's replication ID
            (ReplicationId::Undecided, _) => {
                PeerState::new(self.id.clone(), self.hwm, self.replid.clone(), NodeKind::Replica)
            },
            ...
            _ => PeerState::new(self.id.clone(), self.hwm, self.replid.clone(), NodeKind::NonData),
        }
    }
}
