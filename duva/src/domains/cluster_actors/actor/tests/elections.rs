...
async fn test_run_for_election_transitions_to_candidate_and_sends_request_votes() {
    ...
    let initial_term = actor.replication.state.term;
    ...
    assert_eq!(actor.replication.state.role, ReplicationRole::Follower);
    assert_eq!(actor.replication.state.term, initial_term + 1);
...
async fn test_run_for_election_no_replicas() {
    ...
    assert!(matches!(actor.replication.state.role, ReplicationRole::Follower));
...
async fn test_vote_election_grant_vote() {
    ...
    let initial_term = follower_actor.replication.state.term;
    ...
    assert_eq!(follower_actor.replication.state.term, initial_term + 1);

    assert!(follower_actor.replication.election_votes.votes.contains(&request_vote.candidate_id));
    assert_eq!(follower_actor.replication.state.role, ReplicationRole::Follower); // Stays follower
...
async fn test_vote_election_deny_vote_lower_candidate_term() {
    let follower_term = 3;
    let mut follower_actor = Helper::cluster_actor(ReplicationRole::Follower).await;
    follower_actor.replication.state.term = follower_term;
    ...
    assert_eq!(follower_actor.replication.state.term, follower_term); // Term does not change
...
async fn test_receive_election_vote_candidate_wins_election() {
    ...
    candidate_actor.replication.state.term = candidate_term;
    ...
    assert_eq!(candidate_actor.replication.state.role, ReplicationRole::Leader);
    assert_eq!(candidate_actor.replication.state.term, candidate_term); // Term remains the same as election term
    ...
    let mut hb = HeartBeat {
        ...
        replid: candidate_actor.replication.state.replid.clone(),
...
async fn test_receive_election_vote_candidate_gets_vote_not_enough_to_win() {
    ...
    assert_eq!(candidate_actor.replication.state.role, ReplicationRole::Follower); // Stays follower
