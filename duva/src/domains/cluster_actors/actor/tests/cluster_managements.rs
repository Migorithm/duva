...
async fn test_cluster_nodes() {
    ...
    let repl_id = cluster_actor.replication.logger.state.replid.clone();
    assert_eq!(res.len(), 6);
...
async fn test_reconnection_on_gossip() {
    ...
    cluster_actor
        .join_peer_network_if_absent::<TcpStream>(vec![NodeState {
            ...
            term: cluster_actor.replication.logger.state.term,
        }])
        .await;
...
async fn test_topology_broadcast_on_hash_ring_change() {
    ...
    let hash_ring = HashRing::default()
        .set_partitions(vec![(replid.clone(), PeerIdentifier("peer1".into()))])
        .unwrap();
