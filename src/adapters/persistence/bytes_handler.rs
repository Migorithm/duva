...
make_smart_pointer!(BytesEndec, Vec<u8>);
from_to!(Vec<u8>, BytesEndec);
#[derive(Default)]
pub struct BytesEndec(pub Vec<u8>);
...
fn test_size_decoding() {
    let mut example1: BytesEndec = vec![0x0D].into();
    let mut example2: BytesEndec = vec![0x42, 0xBC].into();
    let mut example3: BytesEndec = vec![0x80, 0x00, 0x00, 0x42, 0x68].into();
    let mut example4: BytesEndec = vec![0xC0, 0x0A].into();
...
fn test_integer_decoding() {
    let mut example1: BytesEndec = vec![0xC0, 0x0A].into();
    let mut example2: BytesEndec = vec![0xC1, 0x39, 0x30].into();
    let mut example3: BytesEndec = vec![0xC2, 0x87, 0xD6, 0x12, 0x00].into();
...
fn test_string_decoding() {
    let mut example1: BytesEndec = vec![
        0x0D, 0x48, 0x65, 0x6C, 0x6C, 0x6F, 0x2C, 0x20, 0x57, 0x6F, 0x72, 0x6C, 0x64, 0x21,
    ]
    .into();
    let mut example2: BytesEndec = vec![0x42, 0x0A, 0x54, 0x65, 0x73, 0x74].into();
...
fn test_decode_multiple_strings() {
    // "abc" and "def"
    let mut encoded: BytesEndec = vec![0x03, 0x61, 0x62, 0x63, 0x03, 0x64, 0x65, 0x66].into();
