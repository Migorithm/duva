...
pub fn string_decode(encoded: &mut Vec<u8>) -> Option<DecodedData> {
...
fn test_decoding() {
    ...
    let mut example2 = vec![0x42, 0x0A, 0x54, 0x65, 0x73, 0x74];

    assert!(string_decode(&mut example1).is_some());
    assert!(string_decode(&mut example2).is_none()); // due to insufficient bytes
}
