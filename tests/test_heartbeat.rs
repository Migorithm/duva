...
use common::{FileName, check_internodes_communication, spawn_server_process};
...
async fn test_heartbeat_hop_count_decreases_over_time() {
    ...
    let mut follower_p1 = spawn_server_process(leader_bind_addr.clone().into(), &file_name);
    let mut follower_p2 = spawn_server_process(leader_bind_addr.clone().into(), &file_name);
    ...
    let mut follower_p3 = spawn_server_process(leader_bind_addr.into(), &file_name);
