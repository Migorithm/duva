...
use common::{
    FileName, check_internodes_communication, spawn_server_as_follower, spawn_server_process,
};
...
async fn test_heartbeat_hop_count_decreases_over_time() {
    ...
    let mut follower_p1 = spawn_server_as_follower(leader_bind_addr.clone(), &file_name);
    let mut follower_p2 = spawn_server_as_follower(leader_bind_addr.clone(), &file_name);
    ...
    let mut follower_p3 = spawn_server_as_follower(leader_bind_addr, &file_name);
