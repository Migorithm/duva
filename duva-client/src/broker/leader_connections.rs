...
mod tests {
    ...
    #[tokio::test]
    async fn test_leader_connections_remove_nonexistent_connection() {
        // Given
        let peer_id = PeerIdentifier::new("localhost", 3333);
        let other_peer = PeerIdentifier::new("localhost", 4444);
        let (tx, _rx) = mpsc::channel(10);
        let (kill_tx, _kill_rx) = oneshot::channel();
        let mut connections = LeaderConnections::new(peer_id.clone(), tx, kill_tx);

        // When
        connections.remove_connection(&other_peer).await;
